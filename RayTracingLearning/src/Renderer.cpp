...
void Renderer::OnResize(uint32_t width, uint32_t height)
{
	if (m_FinalImage)
	{
		//If the image is already the same size, we don't need to resize it so we can return
...
#define MT 1
		...
		[this](uint32_t y)
		{
			...
			std::for_each(std::execution::par, m_ImageHorizontalIter.begin(), m_ImageHorizontalIter.end(),
				[this, y](uint32_t x)
				{
					...
					m_ImageData[x + y * m_FinalImage->GetWidth()] = Utils::ConvertToRGBA(accumulatedColour);
	...
	for (uint32_t y = 0; y < m_FinalImage->GetHeight(); y++) {
		//Goal is to render every pixel of the image
		for (uint32_t x = 0; x < m_FinalImage->GetWidth(); x++) {
...
glm ::vec4 Renderer::PerPixel(uint32_t x, uint32_t y)
{
	...
	for (int i = 0; i < bounces; i++)
	{
		...
		//float lightIntensity = glm::max(glm::dot(payload.WorldNormal, -lightDir), 0.0f); //cosine of the angle between the normal and the light direction
...
Renderer::HitPayload Renderer::TraceRay(const Ray& ray)
{
	...
	// (bx^2 + by^2)t^t + (2(axbx + ayby))t + (ax^2 + ay^2 - r^2) = 0
	// a = ray origin , b = ray direction, r = radius, t = intersection point
